...
export function findInitialUser() {
  ...
  if (Settings.requestSetting('default_user')) {
    ...
    let match = window.lightdm.users.filter(
      user => user.name === settingsDefaultUser
    );
  ...
  if (
    window.lightdm.select_user_hint !== undefined &&
    window.lightdm.select_user_hint !== null
  ) {
    let match = window.lightdm.users.filter(
      user => user.name === window.lightdm.select_user_hint
    );
...
export function findSession(sessionName) {
  ...
  return window.lightdm.sessions.filter(
    session =>
